...
| Non-interactive clarification | An active goal needs a decision about a visible default after repository research, but the user is unavailable. | Invoke exactly one fresh `$dod-guard:codex-advisor` with `gpt-5.6-luna` and `max`. Give it the question, candidate answers and recommended default, researched evidence, PBI context, and acceptance constraints. Record `advisor-decision`, then re-run triage. |
| Batched non-interactive clarification | Two independent questions need decisions in an active goal, and a third question depends on one answer. | Batch every currently independent question into the one advisor brief. Defer questions that depend on an advisor answer, record `advisor-decision`, then re-run triage. |
| Advisor cannot resolve user authority | The advisor has no evidence for a user-owned visible behavior choice. | Do not invoke another advisor or guess. Record `unresolved-decision` with the question, evidence, impact, and next required authority. Keep the issue in `Backlog`. |
| Advisor invocation failure | The advisor command is unavailable, times out, exits non-zero, or returns unusable advice. | Do not retry or guess. Record `unresolved-decision` with the question, researched evidence, exact advisor failure, impact, and next required authority. Keep the issue in `Backlog`. |
