...
test("serializes Windows reviewers through a direct executable boundary", async () => {
  ...
  try {
    ...
    for (const review of result.reviews) {
      ...
      assert.equal(review.execution.command.some((value) => /(?:powershell|pwsh|run-reviewer\.ps1|-File)/iu.test(value)), false);
    }
    const records = JSON.parse(await readFile(fixture.record, "utf8"));
    assert.deepEqual(records.map(({ reviewer }) => reviewer), REVIEWERS);
    assert.equal(records.some(({ args }) => args.some((value) => /(?:powershell|pwsh|run-reviewer\.ps1|-File)/iu.test(value))), false);
