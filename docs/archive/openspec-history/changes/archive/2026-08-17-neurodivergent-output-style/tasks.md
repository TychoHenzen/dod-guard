...
## 1. Create `plugins/natural-output-style/output-styles/neurodivergent.md` - DONE

Create the file with the exact content below. Do not add, remove, or rephrase any rule. The before/after pairs are load-bearing examples, not decoration.
...
**Verify**: The file parses as valid YAML frontmatter, the `name` field is `Neurodivergent`, and `keep-coding-instructions` is `true`.

## 2. Update `plugins/natural-output-style/.claude-plugin/plugin.json`

Change the `description` field to mention both styles. New value:

```
"description": "Two output styles: Natural (plain prose, common words, short sentences, active voice, no filler) and Neurodivergent (tuned for an AuDHD reader with demand avoidance - choice-first, memoryless, no obligations, no body language, interest-first ordering). Both keep Claude Code's built-in coding behavior. No MCP server, no skills, no hooks."
```

Do not change `name` or `version`.

## 3. Update `plugins/natural-output-style/README.md`

Replace the opening line "One output style, `Natural`." with "Two output styles, `Natural` and `Neurodivergent`."

Add a new section after "What the style asks for" documenting what the Neurodivergent style does. Use this text:
...
Update the "What ships" table to list the new file:
...
Find the `natural-output-style` entry and change its `description` to:

```
"description": "Two output styles. Natural: plain prose, common words, short sentences, active voice, no filler. Neurodivergent: tuned for an AuDHD reader with demand avoidance - choice-first, memoryless, no obligations, interest-first ordering. Both keep Claude Code's built-in coding behavior. No MCP server, no skills, no hooks."
```
