...
5. **notify.ts Windows messagebox broken**: `dod_verify` → `showVerifyDialog()` → spawns Windows messagebox → hangs indefinitely (1800s timeout). Human-in-the-loop verification via popup is completely non-functional. All manual/review proofs had to be converted to concrete automated proofs. **FIXED 2026-07-12**: Removed WinForms dialog entirely. `buildConfirmer()` now uses MCP elicitation unconditionally (`showVerifyDialog`, `VERIFY_DIALOG_SCRIPT`, and all PowerShell WinForms spawning deleted from notify.ts). Manual verification now goes through Claude Code's built-in elicitation dialog — same channel already in use for skill permission prompts, so it's battle-tested.
6. **dod_amend predicate type change**: `dod_amend` with `new_predicate` did not change predicate type from manual to exit_code. Had to remove + re-add node. **Investigated 2026-07-12**: Source code is correct — `node.predicate = new_predicate as Predicate` is direct replacement, no merge logic. Bug was likely stale bundle in cache (see S5 below). No code change needed; fixed by proper plugin update workflow.
