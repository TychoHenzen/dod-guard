...
// ── dod_create ──────────────────────────────────────────────────────
server.tool("dod_create", "Create a new locked DoD document. Stores proof commands canonically in MCP storage — editing the rendered markdown cannot weaken verification. Returns the DoD ID for use with dod_check.", {
    ...
    const fpData = dodSteps.flatMap(s => s.proofs.map(p => `${p.command}|${p.predicate.type}|${p.predicate.value ?? ""}`)).join("\n");
...
// ── dod_check ───────────────────────────────────────────────────────
server.tool("dod_check", "Run ALL proof commands for a DoD from canonical (locked) storage, mark pass/fail, update the markdown, and return an overall PASS/FAIL verdict. Use as /goal completion condition. Commands are executed from MCP's locked copy — the markdown file cannot influence results.", {
...
// ── dod_status ──────────────────────────────────────────────────────
server.tool("dod_status", "Get the last check result for a DoD without re-running proofs. Quick read of cached state.", {
...
// ── dod_amend ───────────────────────────────────────────────────────
server.tool("dod_amend", "Modify a proof command in canonical storage with a mandatory audit trail. Use when requirements change and an original proof becomes unreasonable. Resets the proof to pending.", {
    ...
    if (new_command !== undefined)
        proof.command = new_command;
    if (new_predicate !== undefined)
        proof.predicate = new_predicate;
    if (new_description !== undefined)
        proof.description = new_description;
...
// ── dod_import ──────────────────────────────────────────────────────
server.tool("dod_import", "Import an existing DoD markdown file into canonical MCP storage. Parses proof commands, infers predicates, and locks them. Use for pre-existing DoD docs not created via dod_create.", {
