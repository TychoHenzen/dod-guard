...
Defines the evolutionary branching workflow itself. That workflow marks a
point in a repository's history and branches from that mark to try
something. It then reverts a failed attempt, keeps a winning one, and
finishes the run. The
pre-move safety scan belongs to `gitevo/safety-gate`. The durable message
store belongs to `gitevo/memory-bus`. Lesson export belongs to
`gitevo/lesson-export`. This capability names those seams rather than
repeating them.
...
`evo_init` SHALL migrate any lessons recorded in the legacy JSONL file into
the durable message store, then clear the JSONL file. The store SHALL be the
...
exists at the repository's top level. When it does not, the operation SHALL
refuse with a message telling the caller to run `evo_init` first.
...
description. That state can then be resumed by `evo_spawn` or restored by
`evo_abandon`, and the tagging SHALL not disturb the caller's working tree.
...
- **THEN** those changes are committed and the resulting commit is tagged
  `evo-<name>`. The branch is then reset back to the HEAD captured before
  the commit. The caller's working tree ends up dirty again in the same
  way it started
...
checkpoint tag points at. It SHALL refuse when the checkpoint does not exist
...
operation SHALL abort the merge before returning. The repository SHALL be left
as it was before the merge attempt, and the operation SHALL report which
files conflicted.

#### Scenario: Merge produces conflicts
- **WHEN** merging the named branch into the root branch produces conflicting
  files
- **THEN** the merge is aborted and the repository is left unmerged. The error
  names the conflicting files so the caller can resolve them manually or
  abandon the branch
...
already the root branch. It SHALL then remove every `evo-*` tag, delete every
branch other than the root branch and its conventional siblings, and remove
the `.evo/` directory.

#### Scenario: Finish from an attempt branch
- **WHEN** `evo_finish` runs while an attempt branch is active
- **THEN** `evo_finish` adopts that branch into the root branch, then removes
  all evo tags, deletes side branches, and removes `.evo/`
...
work if any is present. Both SHALL refuse to proceed if the stash itself
fails. After spawning, the stash SHALL be popped back onto the new branch. A
failed pop SHALL be reported to the caller rather than silently dropped.
After abandoning, the stash SHALL be left in place rather than popped.
...
process directly, rather than building a shell command line. This way, no
argument needs shell escaping.
...
This way, the working tree carries no gitevo-authored change, and no
working-tree checkpoint ever commits gitevo's own state.
