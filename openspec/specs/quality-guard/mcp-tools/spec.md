...
The server SHALL expose a scan tool that reports violations. It SHALL expose a
gate tool that compares against a baseline and returns a verdict. It SHALL
expose a waiver tool that lists open skip records. Each SHALL return text
content.

#### Scenario: Client lists the tools
- **WHEN** a client asks the server what tools it offers
- **THEN** the server names the scan tool, the gate tool, and the waiver tool,
