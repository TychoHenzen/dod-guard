...
The skill SHALL dispatch `blind-contract-extractor` (for code) or `blind-prose-contract-extractor` (for prose) to extract the target's behavioral contract. The contract SHALL describe behavior in terms of inputs, outputs, and observable effects, never quoting the interior implementation.
...
- **THEN** the skill dispatches `blind-prose-contract-extractor` with claim strength words instead of RFC 2119 keywords
