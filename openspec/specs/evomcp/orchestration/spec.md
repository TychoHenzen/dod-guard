...
SHALL require a harden-pass flag, and MERGE SHALL require a review-pass flag.
A blocked stage SHALL carry a reason describing what is missing.
...
Completing a stage SHALL set the flag(s) that later stages gate on. It SHALL
also record the attempt against that stage's budget, and record a success
against the escalation tracker. Completing SPEC SHALL set the spec-locked
flag. Completing
...
#### Scenario: Completing SPEC starts TEST_AUTHOR
- **WHEN** SPEC completes
...
gate on that command's exit code. A zero exit SHALL pass the stage, and a
nonzero exit SHALL fail the stage.
...
the block reason. The orchestrator SHALL then stop driving further stages.

#### Scenario: Blocked gate halts the run
- **WHEN** advancing the orchestrator finds a stage gate that blocks entry
- **THEN** the orchestrator reports no next stage. If the run had not
...
Driving a playbook run to completion SHALL report exactly one outcome. That
outcome is a pass when the run completed through MERGE, an escalation when
the run was aborted, or incomplete otherwise. The run SHALL also report, per
stage, the stage name, its status, and the time spent in it.
...
The orchestrator SHALL be able to summarize its state as text. That summary
names the current stage, whether the run completed or aborted, and the set
of flags set so far. A run with no flags SHALL report that plainly rather
than an empty list.
