...
Project classification configuration SHALL be an optional `.code-explorer.json` object with `generated`, `test`, `production`, and `overrides` arrays of project-relative glob strings. The file SHALL reject unknown keys, non-string entries, absolute paths, parent traversal, and regular-expression syntax. Within one class, the last matching override SHALL win. If the file is unreadable or invalid, the service SHALL use defaults, report `classification_config_invalid`, and not disable navigation.
