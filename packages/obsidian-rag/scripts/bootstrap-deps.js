...
if (!existsSync(bs3Path)) {
  console.error(`[obsidian-rag] Installing native dependencies...`);
  try {
    ...
    console.error(`[obsidian-rag] Dependencies installed.`);
  } catch (err) {
    console.error(`[obsidian-rag] Failed to install dependencies: ${err.message}`);
    // Don't block startup — MCP server will fail with a clearer error
  }
} else {
  console.error(`[obsidian-rag] Dependencies already present, skipping install.`);
}
