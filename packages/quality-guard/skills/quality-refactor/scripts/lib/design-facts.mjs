...
function parameterName(left, lang) {
  ...
  if (lang === "ts") return /^\.\.\.?(?:\s*)?([A-Za-z_$][\w$]*)/.exec(cleaned)?.[1] ??
    cleaned.split(":", 1)[0].trim().match(/[A-Za-z_$][\w$]*$/)?.[0];
  return cleaned.match(/[A-Za-z_]\w*$/)?.[0];
}

function literalDefault(value) {
  const trimmed = value.trim();
  return /^(?:[-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?[uUlLfFdDmM]*|true|false|null|undefined|none|nil)$/i.test(trimmed) ||
    /^(?:"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')$/s.test(trimmed)
    ? trimmed
    : undefined;
}
  ...
  return functions.flatMap((fn) =>
    parameterText(source, fn).flatMap((parameter) => {
      ...
      return name && value && CONFIGURATION_PARAMETER.test(name)
        ? [{ method: fn.name, parameter: name, defaultValue: value, line: fn.line }]
        : [];
    }),
  );
}
