...
import { formatSkips, readSkipLog } from "../../src/skips.js";

const SKIP_LOG = path.join(".github", "quality", "skip-log.json");

function writeLog(root: string, records: Array<Record<string, unknown>>): void {
  const target = path.join(root, ".github", "quality", "skip-log.json");
  mkdirSync(path.dirname(target), { recursive: true });
  writeFileSync(target, JSON.stringify(records, null, 2));
}
test("the server and the hook agree on where the skip log lives", () => {
  assert.equal(
    path.join(".github", "quality", "skip-log.json"),
    HOOK_SKIP_LOG,
    "a drift here would hide waivers from quality_skips",
  );
});
...
test("formatSkips lists open waivers with their kind and reasons", () => {
  ...
  assert.match(
    out,
    /src\/a\.ts {2}\[rebaseline\] {2}2026-07-30T09:00:00\.000Z/,
  );
  assert.match(out, /complexity: 8 before, 11 now/);
  assert.match(out, /src\/b\.ts {2}\[new-file ceiling\]/);
  assert.equal(
    out.includes("src/done.ts"),
    false,
    "acknowledged waivers must not be listed",
  );
});
