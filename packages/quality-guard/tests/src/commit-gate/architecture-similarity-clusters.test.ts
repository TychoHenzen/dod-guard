...
test("keeps a coherent directory together beyond the similarity trigger", () => {
  const files = Array.from({ length: 25 }, (_, index) => fact(index, "billing"));
  assert.deepEqual(
    analyzeSimilarity({ afterFiles: files, affectedPaths: [files[0].path], config }),
    [],
  );
});
...
test("reports a large-directory outlier with its nearest cluster", () => {
  ...
  assert.deepEqual(finding?.outliers.map((outlier) => outlier.path), ["src/mixed/shipping-24.ts"]);
...
test("reports distinct ownership clusters in a smaller directory", () => {
  ...
  const [finding] = analyzeSimilarity({ afterFiles: files, affectedPaths: [files[0].path], config });
