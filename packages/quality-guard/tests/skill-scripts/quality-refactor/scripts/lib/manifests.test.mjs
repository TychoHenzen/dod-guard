...
import { collectManifests } from "../../../../../skills/quality-refactor/scripts/lib/manifests.mjs";
...
test("collectManifests finds a sibling .tscn file and returns its text", () => {
  withTempDir((dir) => {
    writeFileSync(
      join(dir, "RootScene.tscn"),
      '[node name="TerminalDisplay"]\n',
    );
...
test(
  "collectManifests called with the actual repo root finds a manifest that " +
    "sits ABOVE the scanned target directory",
  () => {
    withTempDir((dir) => {
      ...
      writeFileSync(
        join(targetDir, "TerminalDisplay.cs"),
...
test("collectManifests excludes .md files", () => {
  withTempDir((dir) => {
    writeFileSync(
      join(dir, "README.md"),
      "TerminalDisplay is documented here.\n",
    );
    const manifests = collectManifests(dir);
