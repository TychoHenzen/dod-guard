...
function changedPaths(snapshot: Snapshot): string[] {
  return [...new Set(snapshot.changes.flatMap((change) => [change.before?.path, change.after?.path]).filter((path): path is string => Boolean(path)))].sort(
...
function requiresSourceDecision(snapshot: Snapshot): boolean {
  return changedPaths(snapshot).some((filePath) => SOURCE_PATH.test(filePath) || filePath === QUALITY_CONFIGURATION_PATH);
}

function architectureFinding(kind: string, severity: "review" | "fail", affectedPaths: string[], evidence: FindingEvidence, reason: string): Finding {
