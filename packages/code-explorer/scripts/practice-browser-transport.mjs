...
import { PracticeFailure, readinessTimeoutMs, reconciliationTimeoutMs } from "./practice-browser-config.mjs";
...
export function waitForEndpoint(child) {
  return new Promise((resolvePromise, reject) => {
    ...
    const timer = setTimeout(() => finish(() => reject(new PracticeFailure("practice_start_failed"))), readinessTimeoutMs);
...
export async function call({ page, endpoint, session, tab, route, body }) {
  return page.evaluate(
    async ({ endpoint: url, session: sessionId, tab: tabId, route: path, body: requestBody }) => {
      const response = await fetch(`${url}${path}`, { method: "POST", headers: { "content-type": "application/json", "x-code-explorer-session": sessionId, "x-code-explorer-tab": tabId }, body: JSON.stringify(requestBody) });
...
export function sameLocation(actual, expected, path) {
  return actual?.path === path && actual?.range?.start?.line === expected?.start?.line && actual?.range?.start?.character === expected?.start?.character && actual?.range?.end?.line === expected?.end?.line && actual?.range?.end?.character === expected?.end?.character;
}

export async function waitForGeneration({ page, endpoint, session, tab, startGeneration }) {
  const deadline = Date.now() + reconciliationTimeoutMs;
  while (Date.now() <= deadline) {
    const status = expectSuccess(
      await call({ page, endpoint, session, tab, route: "/api/status", body: { action: "status" } }),
      "status",
    );
