...
export async function reconcile({ page, endpoint, session, tab, root, oracle, evidence, startGeneration, candidate, focus, handle }) {
  ...
  const stale = await call({ page, endpoint, session, tab, route: "/api/follow", body: { request_id: randomUUID(), view_id: focus.view_id, handle: handle.handle, relation: "callers" } });
  ...
  const refocus = await call({ page, endpoint, session, tab, route: "/api/focus", body: { request_id: randomUUID(), symbol_id: candidate.identity } });
  recordRefocus(refocus, evidence);
  const refreshPayload = expectSuccess(await call({ page, endpoint, session, tab, route: "/api/status", body: { action: "refresh", request_id: randomUUID() } }), "refresh");
