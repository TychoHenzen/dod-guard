...
it("returns a structured unknown-tool error without changing state", async () => {
  ...
  assert.deepEqual(result, { schema_version: 1, code: "unknown_tool", message: "unknown_tool", retryable: false });
  ...
  try {
    ...
    const response = (await client.callTool({ name: "rename", arguments: {} })) as { isError?: boolean; content: Array<{ text: string }> };
    assert.equal(response.isError, true);
    assert.deepEqual(JSON.parse(response.content[0]?.text ?? ""), { schema_version: 1, code: "unknown_tool", message: "unknown_tool", retryable: false });
  } finally {
    await client.close();
  }
});
...
it("returns error envelopes through structuredContent and matching JSON text", async () => {
  ...
  try {
    await client.connect(transport);
    const response = (await client.callTool({ name: "not_a_tool", arguments: {} })) as { isError?: boolean; structuredContent: unknown; content: Array<{ text: string }> };
    assert.equal(response.isError, true);
