...
function request(
  ...
  return router.handle({
    ...
    body: Buffer.from(
      input.body ?? JSON.stringify({ action: "create", tab_instance_id: "tab", document_start: "new" }),
    ),
  });
}
...
describe("browser HTTP boundary", () => {
  ...
  it("rejects unknown closed-body fields", async () => {
    ...
    const response = await request(router, {
      body: JSON.stringify({ action: "create", tab_instance_id: "tab", document_start: "new", extra: true }),
    });
  ...
  it("counts API bytes before JSON decoding", async () => {
    ...
    const allowed = await request(router, {
      path: "/api/status",
      body: `${prefix}${"x".repeat(65_536 - Buffer.byteLength(prefix) - Buffer.byteLength(suffix))}${suffix}`,
    });
  ...
  it("maps allowed navigation and redacted core errors", async () => {
    ...
    const response = await request(router, {
      ...
      headers: { "x-code-explorer-session": session, "x-code-explorer-tab": "tab" },
    });
  ...
  it("preserves a successful status envelope from the shared core", async () => {
    const router = new BrowserHttpRouter({
      ...
      call: async (_name: string, arguments_: Record<string, unknown>) =>
        arguments_.action === "start_session"
          ? {
              schema_version: 1,
              project_id: "p",
    ...
    const response = await request(router, {
      path: "/api/status",
      body: JSON.stringify({ action: "status" }),
      headers: { "x-code-explorer-session": browserSession, "x-code-explorer-tab": "tab" },
    });
  ...
  it("does not reveal another tab session and expires at the exact monotonic boundary", async () => {
    ...
    const rejected = await request(router, {
      path: "/api/status",
      body: JSON.stringify({ action: "status" }),
      headers: { "x-code-explorer-session": session, "x-code-explorer-tab": "other" },
    });
    assert.equal(JSON.parse(rejected.body).code, "invalid_browser_session");
    now = 1_800_000;
    const expired = await request(router, {
      path: "/api/status",
      body: JSON.stringify({ action: "status" }),
      headers: { "x-code-explorer-session": session, "x-code-explorer-tab": "tab" },
    });
