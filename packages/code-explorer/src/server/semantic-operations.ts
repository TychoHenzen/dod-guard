...
export async function collectSemanticSymbols(
  ...
  const replies = await Promise.allSettled(
    adapters.map((adapter) =>
      run(() => adapter.request({ operation: "search", query })),
    ),
  );
  const symbols = replies.flatMap((reply) =>
    reply.status === "fulfilled" && reply.value.operation === "search"
      ? reply.value.symbols
      : [],
  );
  const failure = replies.find(
    (reply): reply is PromiseRejectedResult => reply.status === "rejected",
  )?.reason;
  return { symbols, failure };
}

export async function collectFocusedSymbol(
  adapters: readonly LanguageAdapter[],
  symbolId: string,
  run: BackendOperation,
) {
  const replies = await Promise.allSettled(
    adapters.map((adapter) =>
      run(() => adapter.request({ operation: "focus", symbol_id: symbolId })),
    ),
  );
  const focused = replies.find(
    (
      ...
      Extract<
        Awaited<ReturnType<LanguageAdapter["request"]>>,
        { operation: "focus" }
      >
...
export function throwBackendLimitFailure(
  replies: readonly PromiseSettledResult<unknown>[],
): void {
