...
import { createFocusView, FocusBodyLimitError, type FocusView } from "../navigation/focus-view.js";
...
export async function handleFocus(
  ...
  const selected = await collectFocusedSymbol(runtime.options.adapters ?? [], focus.symbol_id, (operation) =>
    runtime.backendRequests.run(focus.session_id, operation),
  );
