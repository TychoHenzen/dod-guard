...
export function createRuntimeServer(
  ...
  return createCodeExplorerServer({
    ...
    freshness: createNativeWorkspaceFreshness({
      ...
      supported: (candidate) =>
        /\.(?:rs|py|cs|ts|tsx|js|jsx|json)$/iu.test(candidate),
    }),
  });
}
...
function createRuntimeCore(
  ...
  return {
    ...
    connect: (transport: Transport) => server.mcp.connect(transport),
    close: async (signal) => {
...
async function startRuntimeCore({
  projectRoot,
  signal,
}: Parameters<ExplorerCoreFactory["start"]>[0]): Promise<ExplorerCore> {
  loadAdapterSelectionRecord();
