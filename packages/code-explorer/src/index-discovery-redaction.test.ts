...
import { adapterWithSymbols, symbol } from "./testing/index-test-server-support.js";
...
it("normalizes Windows-form backend paths in discovery responses", async () => {
  ...
  try {
    ...
    const server = createServer({ projectRoot: createNativeProjectRoot(root), adapters: [adapterWithSymbols([symbol("function", "src\\Helper.rs")])] });
...
it("rejects an out-of-project backend location without exposing its path", async () => {
  ...
  try {
    ...
    const server = createServer({ projectRoot: createNativeProjectRoot(root), adapters: [adapterWithSymbols([symbol("function", join(outside, "Helper.rs"))])] });
    const result = await server.call("code_search", { query: "helper" });
    assert.deepEqual(result, { schema_version: 1, code: "path_outside_project", message: "path_outside_project", retryable: false });
...
it("removes sensitive adapter symbols before discovery classification, filters, limits, and responses", async () => {
  ...
  try {
    const adapter = adapterWithSymbols([symbol("function", ".env"), symbol("function", ".git/config"), symbol("function", "keys\\nested\\key.pem"), symbol("function", "src/Helper.rs")]);
    ...
    for (const result of [...searches, status]) {
      assert.equal("code" in result, false);
      if ("code" in result) throw new Error("expected discovery response");
      assert.doesNotMatch(JSON.stringify(result), /\.env|\.git\/config|key\.pem/iu);
    }
    for (const result of searches) {
      if ("code" in result) throw new Error("expected discovery response");
      assert.deepEqual((result.data.candidates as { path: string }[]).map((candidate) => candidate.path), ["src/Helper.rs"]);
    }
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
