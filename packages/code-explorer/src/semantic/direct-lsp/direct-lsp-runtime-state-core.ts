...
export class DirectLspRuntimeStateCore {
  ...
  get process() { return this.#life.process; }
  get epoch() { return this.#life.epoch; }
  get state() { return this.#life.state; }
  get stopping() { return this.#life.stopping; }
  get stopped() { return this.#life.stopped; }
  get serverCapabilities() { return this.#docs.serverCapabilities; }
  ...
  setState(state: DirectLspStatus["state"]) { this.#life.setState(state); }
  markStopped() { this.#life.markStopped(); }
  setStopping(stopping: boolean) { this.#life.setStopping(stopping); }
  setExitResolver(resolve?: () => void) { this.#life.setExitResolver(resolve); }
  resolveExit() { this.#life.resolveExit(); }
  clearExitResolver() { this.#life.clearExitResolver(); }
  killProcess() { this.#life.killProcess(); }
  appendBytes(chunk: Uint8Array) { this.#buffers.append(chunk); }
  bytesLength() { return this.#buffers.length(); }
  bytesSlice(start?: number, end?: number) {
    return this.#buffers.slice(start, end);
  }
  consumeBytes(end: number) { this.#buffers.consume(end); }
  nextRequestId() { return this.#req.nextId(); }
  setPending(id: number, pending: Pending) {
    this.#req.setPending(id, pending);
  }
  pending(id: number) { return this.#req.pending(id); }
  deletePending(id: number) { return this.#req.deletePending(id); }
  ...
  eventsSnapshot() { return this.#telemetry.eventsSnapshot(); }
  restartDelaysSnapshot() { return this.#telemetry.restartDelaysSnapshot(); }
  recordCrash() { return this.#restarts.recordCrash(); }
  scheduleRestart(callback: () => void, delay: number) {
    this.#restarts.schedule(callback, delay);
  }
  cancelRestarts() { this.#restarts.cancel(); }
  recordTimeout() { return this.#restarts.recordTimeout(); }
  resetFailureHistory() {
  ...
  hasOpenedDocument(uri: string) { return this.#docs.hasOpened(uri); }
  markDocumentOpened(uri: string) { this.#docs.markOpened(uri); }
  current(epoch: number) { return this.#life.current(epoch); }
