...
import { startSession } from "./testing/index-test-session-support.js";
...
it("refreshes only derived state and preserves view history", async () => {
  ...
  const result = await server.call("code_status", { action: "refresh", session_id: sessionId, request_id: "refresh-request-0001" });
