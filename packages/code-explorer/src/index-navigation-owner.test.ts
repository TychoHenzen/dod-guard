...
import { followDefinition, startSession, symbol } from "./testing/index-test-server-support.js";
...
it("keeps the owning language relation when another ready adapter does not own the symbol", async () => {
  ...
  const owner: LanguageAdapter = {
    status: () => status("rust"),
    request: async (request) => request.operation === "focus"
      ? { operation: "focus", revision: { generation: 1, manifest_sha256: "test" }, symbol: focused, content: { body: "helper", visible_symbols: [{ name: "helper", symbol_id: focused.id }] } }
      : { operation: "definition", revision: { generation: 1, manifest_sha256: "test" }, relations: [{ relation: "definition", symbol: focused, location: focused.location }] },
  } as LanguageAdapter;
  const unrelated: LanguageAdapter = {
    status: () => status("python"),
    request: async () => { throw new Error("backend_unavailable"); },
  };
  const server = createServer({ adapters: [owner, unrelated] });
  const sessionId = await startSession(server);
  const focus = await server.call("code_focus", { session_id: sessionId, request_id: "focus-request-00001", symbol_id: focused.id });
