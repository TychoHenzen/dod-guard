...
/** Identifies the root-only configuration using the host filesystem's case
 * rules.
 */
export function isClassificationConfigPath(
  path: string,
  platform = process.platform,
): boolean {
  if (path.includes("/") || path.includes("\\")) return false;
  return platform === "win32"
    ? path.toLocaleLowerCase("en-US") === configName
    : path === configName;
}
...
export function findClassificationConfigPath(
  ...
  return readdirSync(projectRoot, { withFileTypes: true }).find(
    (entry) =>
      entry.isFile() && isClassificationConfigPath(entry.name, platform),
  )?.name;
}

export function classificationConfigPath(
  projectRoot: string,
  platform = process.platform,
): string | undefined {
