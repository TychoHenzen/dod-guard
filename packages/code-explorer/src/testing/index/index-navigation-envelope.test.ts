...
import {
  followDefinition,
  focusableNavigationAdapter,
  startSession,
} from "./index-test-server-support.js";

it(
  "returns only the common versioned envelope for every successful " + "tool",
  async () => {
...
it(
  "makes an unsupported relation explicit instead of returning an empty " +
    "result array",
  async () => {
    const server = createServer({ adapters: [focusableNavigationAdapter()] });
    const sessionId = await startSession(server);
    ...
    for (const [
      index,
      relation,
    ] of "definition,references,callers,callees,type,implementation"
      .split(",")
      .entries()) {
      ...
      assert.equal("code" in result, false);
      if ("code" in result)
        throw new Error("expected an unavailable relation result");
      assert.equal(result.state, "unavailable_relation");
      assert.deepEqual(result.data, { relation });
      assert.equal(
        Array.isArray((result.data as Record<string, unknown>).results),
        false,
      );
    }
  },
