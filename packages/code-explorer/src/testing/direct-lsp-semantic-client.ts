import type { DirectLspStatus, ProtectedDocumentContent } from "../semantic/direct-lsp/direct-lsp.js";
...
export function semanticClient(
  ...
  return {
    status: () => ({ state: "ready" as const, events: [], restart_delays_ms: [], server_capabilities: serverCapabilities }),
    request: (method: string, params: unknown) => Promise.resolve(request(method, params)),
