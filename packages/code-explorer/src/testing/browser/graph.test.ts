...
import { type GraphRelationGroup, projectOneHopGraph, renderOneHopGraph } from "../../browser/graph.js";
import { BrowserGraphController, graphFor, graphSnapshot, renderGraphArea, toGraphRelationGroups } from "../../browser/graph-navigation.js";
import { BrowserViewHistory } from "../../browser/history.js";
...
describe("graph navigation and view ownership", () => {
  it("routes a local graph node through focus navigation and resets the graph after success", async () => {
    ...
    assert.deepEqual(
      graphFor({ symbol_id: "project::Caller", name: "New" }, []).nodes.map((node) => node.symbol_id),
      ["project::Caller"],
    );
  });
