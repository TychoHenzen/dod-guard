...
import { createBrowserStore, renderBrowserShell } from "../../browser/app.js";

describe("browser shell", () => {
  it(
    "renders landmarks, an empty focus " + "prompt, and no invented relations",
    () => {
      const html = renderBrowserShell(
        createBrowserStore({
          landmarks: [{ group: "Files", items: ["src/lib.rs"] }],
        }).state(),
        1200,
      );
  ...
  it(
    "keeps a focused symbol central " +
      "while both side panes remain available",
    () => {
      const store = createBrowserStore();
      store.dispatch({
        operation: "focus",
        symbol: { name: "parse", path: "src/lib.rs", kind: "function" },
      });
  ...
  it(
    "renders mouse-operable side " + "drawers below the desktop breakpoint",
    () => {
  ...
  it(
    "maps every visible action to a " +
      "read-only navigation or local operation",
    () => {
      const store = createBrowserStore();
      for (const operation of store.visibleOperations())
        assert.doesNotThrow(() => store.dispatch({ operation }));
      assert.deepEqual(
  ...
  it("rejects an operation outside " + "the closed browser action set", () => {
    const store = createBrowserStore();
    assert.throws(
      () => store.dispatch({ operation: "write_file" }),
      /unsupported_browser_operation/,
    );
  });

  it(
    "keeps refresh available while " +
      "generation-zero state disables navigation controls",
    () => {
      const html = renderBrowserShell(
        createBrowserStore({
