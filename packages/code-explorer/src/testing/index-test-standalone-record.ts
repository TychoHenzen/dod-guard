...
function runtimeBackend(
  ...
  return {
    ...
    sentinel_evidence: { fixture: `fixtures/${language}`, platform: "win32", fixture_sha256: fixtureHash, side_effect_absent: true, result: "passed", passed: true },
    authorization,
  };
}
...
function evidencePlatforms(version: string) {
  return {
    win32: { status: "passed", command_roots: ["code_explorer_backends"], commands: ["test fixture"], bounded_output: "test fixture", backend_versions: backendVersions(version), positive_controls: passedPositiveControls() },
    posix: { status: "unproven", command_roots: ["posix_code_explorer_backends"], commands: [], bounded_output: "not run", backend_versions: { rust: null, python: null, csharp: null }, positive_controls: unprovenPositiveControls() },
  };
}

function evidenceRecord(version: string, fixtureHashes_: Record<string, string>, runs: ReturnType<typeof sentinelRuns>) {
  return {
    schema_version: 1,
