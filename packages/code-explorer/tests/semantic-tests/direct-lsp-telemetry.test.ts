...
import { DirectLspRuntimeTelemetry } from "../../src/semantic/direct-lsp/direct-lsp-runtime-telemetry.js";

it("bounds retained restart delay telemetry", () => {
  const telemetry = new DirectLspRuntimeTelemetry();
  for (let delay = 0; delay < 300; delay += 1)
    telemetry.recordRestartDelay(delay);
