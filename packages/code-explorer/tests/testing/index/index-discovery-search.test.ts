...
it(
  "uses the production search boundary to filter generated files and " +
    "report invalid classification configuration",
  async () => {
    ...
    try {
      ...
      if ("code" in search || "code" in status)
        throw new Error("expected discovery response");
      assert.deepEqual(
        (search.data.candidates as { path: string }[]).map(
...
it(
  ...
  async () => {
    ...
    try {
      ...
      assert.deepEqual(
        (result.data.candidates as { identity: string }[]).map(
