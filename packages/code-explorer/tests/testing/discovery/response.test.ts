...
it(
  "returns bounded candidates with exact omissions and " +
    "available narrowing filters",
  () => {
    const root = mkdtempSync(
      join(tmpdir(), "code-explorer-discovery-response-"),
    );
    try {
      ...
      const response = createDiscoveryPipeline(
        createNativeProjectRoot(root),
      ).searchResult("helper", { limit: 1 });
...
it("returns an honest empty response with " + "the applied filters", () => {
  ...
  try {
    ...
    assert.deepEqual(response.applied_filters, {
      content: "production",
      kinds: ["function"],
    });
  } finally {
