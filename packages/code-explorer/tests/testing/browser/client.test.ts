...
// test-sources: ../../../src/browser/client.ts ../../../src/browser/application.ts ../../../src/browser/application-navigation.ts

test(
  "reports the browser server " + "state in the application root",
  async (context) => {
    const fixture = installClientFixture();
    context.after(fixture.restore);

    const modulePath = `../../../../dist/browser/client.js?test=${Date.now()}`;
...
test(
  "keeps an inaccessible project " + "root in an unavailable local state",
  async (context) => {
    const fixture = installClientFixture({
      rootAccess: "project_root_inaccessible",
    });
    context.after(fixture.restore);

    await import(
      `../../../../dist/browser/client.js?root-access=${Date.now()}`
    );
    await new Promise<void>((resolve) => setImmediate(resolve));
