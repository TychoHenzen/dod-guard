...
import { BrowserRelationsController } from "../../../../src/browser/relations.js";

describe("lazy browser relations", () => {
  it(
    "starts supported and unavailable " +
      "groups without eager relation requests",
    () => {
      ...
      const relations = new BrowserRelationsController(
        {
          ...
          supported: ["definition", "references"],
          unavailable: ["callers"],
        },
        async (request) => {
          calls.push(request);
  ...
  it(
    "dispatches one bounded follow request " +
      "for an opened group and caches its result",
    async () => {
      const calls: Record<string, unknown>[] = [];
      const relations = new BrowserRelationsController(
        {
...
const referencesReply = {
  state: "ok",
  data: {
    candidates: [{ name: "reference", external: false }],
    omitted_count: 3,
  },
};
