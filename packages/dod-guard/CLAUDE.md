...
| `quality-refactor` | Systematic refactoring to an explicit machine-checked quality bar. Ships `scripts/quality-scan.mjs` (zero-dep structural scanner) and emits a `.step-session/steps.json` plan for `/step-by-step` |
