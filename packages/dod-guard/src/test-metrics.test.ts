...
describe("scoreFromMetrics", () => {
  ...
  it("penalizes determinism violations", () => {
    const m = baseMetrics({
      hasRealTime: true, hasUnseededRandom: true, hasSleep: true,
      hasRealFilesystem: true, hasRealNetwork: true, hasRealDatabase: true,
    });
    const s = scoreFromMetrics(m);
  ...
  it("penalizes shared mutable state (half point)", () => {
    ...
    assert.ok(dirty.determinism < clean.determinism,
      `shared mutable should drop determinism: ${dirty.determinism} < ${clean.determinism}`);
  });

  it("scores empty test file", () => {
    const m = baseMetrics({
      testFunctions: [], testFunctionCount: 0, totalAssertions: 0,
      trivialAssertions: 0, truthinessAssertions: 0, specificAssertions: 0,
      assertionsWithMessage: 0, assertionsWithoutMessage: 0,
      zeroAssertionFunctions: 0, errorPathFunctions: 0,
    });
    const s = scoreFromMetrics(m);
  ...
  it("rewards setup/teardown in isolation", () => {
    ...
    assert.ok(withSt.isolation > withoutSt.isolation,
  ...
  it("penalizes module mutables beyond threshold", () => {
    ...
    assert.ok(high.isolation < low.isolation,
      `6 mutables ${high.isolation} < 1 mutable ${low.isolation}`);
  });
  ...
  it("penalizes hasTestOnly in isolation", () => {
    ...
    assert.ok(dirty.isolation < clean.isolation,
  ...
  it("rewards AAA markers in clarity", () => {
    ...
    assert.ok(withAaa.clarity > withoutAaa.clarity,
  ...
  it("penalizes generic test names in clarity", () => {
    ...
    const generics = scoreFromMetrics(baseMetrics({
      genericNames: ["test1", "test2", "test the thing"],
    }));
    assert.ok(generics.clarity < clean.clarity,
  ...
  it("penalizes magic numbers heavily beyond 1", () => {
    ...
    assert.ok(many.clarity < few.clarity,
      `8 magic numbers ${many.clarity} < 1 magic number ${few.clarity}`);
  });
  ...
  it("penalizes sleep/wait calls in speed", () => {
    ...
    assert.ok(yesSleep.speed < noSleep.speed,
      `sleep should drop speed: ${yesSleep.speed} < ${noSleep.speed}`);
  });
  ...
  it("penalizes real I/O in speed", () => {
    ...
    assert.ok(yesIo.speed < noIo.speed,
      `real I/O should drop speed: ${yesIo.speed} < ${noIo.speed}`);
  });
  ...
  it("rewards error path coverage", () => {
    ...
    assert.ok(withErr.coverage_depth >= noErr.coverage_depth,
  ...
  it("all scores within 1-10 range for edge cases", () => {
    ...
    const keys = ["assertion_quality", "determinism", "isolation", "clarity",
      "coverage_depth", "speed", "diagnostics", "assertion_triviality", "overall"] as const;
    for (const k of keys) {
  ...
  it("perfect file (all best values) scores near 10", () => {
    const perfect = baseMetrics({
      ...
      hasRealTime: false, hasUnseededRandom: false, hasSleep: false,
      hasRealFilesystem: false, hasRealNetwork: false, hasRealDatabase: false,
      hasSharedMutableState: false,
      hasSetupTeardown: true,
...
describe("analyseTestMetrics", () => {
  ...
  before(() => { tmp = tmpDir(); mkdir(tmp); });
  after(() => { try { rmSync(tmp, { recursive: true, force: true }); } catch { /* cleanup best-effort */ } });
  ...
  it("detects generic test names", () => {
    writeFile(tmp, "generics.test.ts",
      "import { describe, it } from 'node:test';\n" +
      "import assert from 'node:assert/strict';\n" +
  ...
  it("detects error path test functions", () => {
    ...
    assert.ok(report !== null);
    const m = report!.perFile[0]!;
    assert.ok(m.errorPathFunctions > 0,
      `expected error path function detected, got ${m.errorPathFunctions}`);
  });

  it("detects hasRealTime when Date.now called without mock", () => {
    writeFile(tmp, "realtime.test.ts",
      "import { describe, it } from 'node:test';\n" +
      "import assert from 'node:assert/strict';\n" +
      "it('timestamps', () => {\n" +
      "  const now = Date.now();\n" +
  ...
  it("skips files in dist/ and build/ directories", () => {
    mkdir(tmp, "dist");
    writeFile(tmp, "dist/skipme.test.ts",
  ...
  it("handles glob patterns in command", () => {
    ...
    mkdir(globDir);
    try {
      writeFile(globDir, "glob-a.test.ts",
        "import { it } from 'node:test';\nimport assert from 'node:assert/strict';\nit('a', () => { assert.equal(1, 1); });\n");
      writeFile(globDir, "glob-b.test.ts",
      ...
      try { rmSync(globDir, { recursive: true, force: true }); } catch { /* cleanup */ }
    }
  });
});
