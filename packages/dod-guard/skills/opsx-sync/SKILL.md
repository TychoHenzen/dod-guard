---
name: opsx-sync
description: Sync delta specs from an OpenSpec change to main specs, with intelligent merging of ADDED/MODIFIED/REMOVED/RENAMED requirements. Use when the user wants to update main specs with changes from a delta spec, without archiving the change.
---
...
3. **Find delta specs**
   ...
   `existingOutputPaths` is empty, report that no delta specs exist to sync.
   Do not infer them from other artifacts. Stop without requesting artifact
   ...
   Then sync only the named paths and leave the rest untouched. Bulk
   archive excludes a delta whose implementation it could not find.
   Syncing it anyway would write a main spec the caller withheld.
   Carry that narrowed selection through the merge step. Never widen it
   back to the full list. If a named path is not in `existingOutputPaths`, do not sync it -
...
4. **For each delta spec, apply changes to main specs**
   ...
   For each capability delta spec path from the find-delta-specs step -
   the full `existingOutputPaths` list, or the narrowed subset when a
   caller supplied one (these may belong to a selected store, not the
   repo):
   ...
   c. **Apply changes intelligently**:
      ...
      **REMOVED Requirements:**
      ...
      - Retiring the capability. Delete the whole `spec.md` - and the directory once
        ...
        4. every other nonblank line in the whole file belongs to the title,
           Purpose, Requirements header, or a canonical requirement block;
   ...
   d. **Create new main spec** if capability doesn't exist yet:
      ...
      - Add Purpose section. Copy the delta's `## Purpose` body verbatim
        when it has one (`openspec archive` does the same). Write a brief
        TBD placeholder when it does not.
...
Main specs are what the delta merges INTO. They live at
`openspec/specs/<group>/<capability>/spec.md` and must never contain delta
...
- Never copy a delta file into a main spec as-is. Merge its content so
  the main spec keeps the structure shown in the format reference above,
  with no delta operation headers.
