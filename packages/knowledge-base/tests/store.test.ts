...
test("stays isolated from a separate memory directory", async () => {
  const root = await exampleRoot();
  try {
    const memoryDir = join(root, "built-in-memory");
    await mkdir(memoryDir, { recursive: true });
    const sentinel = join(memoryDir, "secret.md");
    await writeFile(sentinel, "memsd9z2q7only", "utf8");
    const base = new KnowledgeBase(root);
    assert.equal(readFileSync(sentinel, "utf8"), "memsd9z2q7only");
    assert.deepEqual(await base.search("memsd9z2q7only"), []);
  } finally {
    await removeRoot(root);
  }
});
