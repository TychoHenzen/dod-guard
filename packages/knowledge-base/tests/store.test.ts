...
test("indexes, searches, and reads the shipped corpus", async () => {
  ...
  assert.match(
    (await base.get("clean-code.objects-data-structures")).sources[0]?.label ?? "",
    /PDF pages 124-132/,
  );
  ...
  assert.match(
    (await base.get("clean-code.refactoring-serialdate")).sources[0]?.label ?? "",
    /PDF pages 299-316/,
  );
  assert.match(
    (await base.get("clean-code.successive-refinement")).sources[0]?.label ?? "",
    /PDF pages 225-282/,
  );
