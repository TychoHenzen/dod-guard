...
// better-sqlite3 is a native module (needed transitively via bundled gitevo
// code) excluded from esbuild bundle, so it must be installed in node_modules.
//
...
if (!existsSync(bs3Path)) {
  console.error(`[evomcp] Installing native dependencies...`);
  try {
    ...
    console.error(`[evomcp] Dependencies installed.`);
  } catch (err) {
    console.error(`[evomcp] Failed to install dependencies: ${err.message}`);
    // Don't block startup — MCP server will fail with a clearer error
  }
} else {
  console.error(`[evomcp] Dependencies already present, skipping install.`);
}
