...
test("skips oversized sources before reading content and retains later bounded sources", () => {
    ...
    const sources = [
        ...
        { path: "src/oversized.ts", language: "typescript" },
        { path: "src/later.ts", language: "typescript" },
    ];
    const sizes = new Map([
    ...
    const result = readBoundedReferenceSources(sources, (source) => ({ byteLength: sizes.get(source.path) ?? 0 }), (source) => {
        contentReads.push(source.path);
        return `// ${source.path}\n`;
    });
    assert.deepEqual(contentReads, ["src/exact.ts", "src/later.ts"]);
    assert.deepEqual(result.sources.map((source) => source.path), ["src/exact.ts", "src/later.ts"]);
...
test("omits binary content from bounded reference sources and continues with later text", () => {
    ...
    const result = readBoundedReferenceSources([
        ...
        { path: "src/later.ts", language: "typescript" },
    ], () => ({ byteLength: 12 }), (source) => sourceContents.get(source.path) ?? "");
...
test("resolves one namespace-level C# using to its unique current path suffix", () => {
    ...
    assert.deepEqual(graph.edges.map(({ sourcePath, targetPath, language, kind, strength, span }) => ({
        sourcePath,
        targetPath,
        language,
        kind,
...
test("retains all sorted C# namespace matches as unresolved evidence", () => {
    ...
    assert.deepEqual(graph.unresolved.map(({ sourcePath, targetCandidates, language, kind, resolution, span }) => ({
        sourcePath,
        targetCandidates,
        language,
        kind,
...
test("keeps an ordinary imported candidate use as a strong inbound reference", () => {
    ...
    assert.deepEqual(graph.edges.map(({ sourcePath, targetPath, strength }) => ({ sourcePath, targetPath, strength })), [{ sourcePath: "src/live.ts", targetPath: "src/candidate.ts", strength: "strong" }]);
});
test("marks imports used only in balanced try or catch bodies as weak", () => {
    ...
    assert.deepEqual(graph.edges.map((edge) => edge.strength), ["weak", "weak", "weak", "weak", "weak"]);
});
...
test("marks C# and Rust uses confined to guards as weak", () => {
    ...
    assert.deepEqual(graph.edges.map((edge) => edge.strength), ["weak", "strong", "weak", "strong", "strong"]);
});
