...
import { analyzeJavaScriptReferences } from "../src/ref-analyzer.js";
import { edgeSummary } from "./ref-analyzer.test-support.js";

test(
  "resolves static, require, and dynamic relative module forms against " +
    "current files",
  () => {
    ...
    assert.deepEqual(
      graph.unresolved.map(
        ({ sourcePath, targetCandidates, language, kind, resolution }) => ({
          sourcePath,
          targetCandidates,
          language,
          kind,
