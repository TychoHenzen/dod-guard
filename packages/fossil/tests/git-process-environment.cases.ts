...
import { discoverGitRepository, type GitSpawn } from "../src/git-process.js";
import { repositoryDiscoveryArguments } from "./git-process.test-support.js";

test(
  "overrides hostile pager and prompt settings while preserving unrelated " +
    "environment values",
  () => {
    const calls: Array<{
...
test(
  "adds config overrides that disable repository filesystem monitors and " +
