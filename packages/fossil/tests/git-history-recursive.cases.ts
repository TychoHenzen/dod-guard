...
import { splitAtChangePoint } from "../src/git-analyzer.js";
import {
  changePointCommits,
  changePointPartitionLengths,
} from "./git-analyzer.test-support.js";

test(
  "ranks close change points deterministically and recursively splits both " +
    "sides",
  () => {
    ...
    assert.deepEqual(
      recursivePartitions.map((partition) =>
        partition.map((commit) => commit.hash),
      ),
      Array.from({ length: 6 }, (_, group) =>
        Array.from({ length: 5 }, (_, offset) => `point-${group * 5 + offset}`),
      ),
    );
  },
);
