...
import { workspaceDebrisFinding } from "../src/workspace-debris.js";

test(
  "reports an unreferenced old candidate as separate " + "workspace debris",
  () => {
...
test(
  "labels a high-confidence-looking debris candidate for review with mtime " +
    "uncertainty",
  () => {
    const finding = workspaceDebrisFinding({
      candidate: {
    ...
    assert.equal(
      finding.ageUncertainty.includes(
        "Copying, restoring, extracting, or rebuilding can change it.",
      ),
      true,
    );
    ...
    assert.equal(
      JSON.stringify(finding).toLowerCase().includes("delete"),
      false,
    );
  },
);
