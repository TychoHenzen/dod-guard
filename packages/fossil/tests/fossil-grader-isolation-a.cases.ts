...
import { clusterIsolationScore } from "../src/fossil-grader.js";
import type { ReferenceGraph } from "../src/types.js";
import { referenceEdge } from "./fossil-grader.test-support.js";

test("gives full isolation for fossil candidate neighbors", () => {
  ...
  assert.equal(
    clusterIsolationScore(
      "src/candidate.ts",
      graph,
      new Set(["src/fossil-inbound.ts", "src/fossil-outbound.ts"]),
    ),
    1,
  );
});
test("gives full isolation without qualifying neighbor evidence", () => {
  const graph: ReferenceGraph = {
    edges: [
      {
        sourcePath: "src/candidate.ts",
