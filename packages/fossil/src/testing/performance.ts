...
export function fastImportStream({ commitCount, fileCount }: PerformanceFixtureSpec): string {
...
function runFastImport(root: string, input: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    child.once("close", (code) => {
      if (code === 0) resolve();
      else reject(new Error(`git fast-import failed with exit code ${code ?? "unknown"}: ${stderr}`));
    });
    child.stdin.end(input);
  });
}

function pointHeadAtImportedBranch(root: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    child.once("error", reject);
    child.once("close", (code) =>
      code === 0 ? resolve() : reject(new Error(`git symbolic-ref failed with exit code ${code ?? "unknown"}`)),
    );
  });
}
...
export async function createPerformanceFixture(
  spec: PerformanceFixtureSpec = TARGET_PERFORMANCE_FIXTURE,
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      child.once("error", reject);
      child.once("close", (code) =>
        code === 0 ? resolve() : reject(new Error(`git init failed with exit code ${code}`)),
      );
    });
...
export async function benchmarkPerformanceFixture(
  ...
  { runFreshJsonAnalysis, now = performance.now.bind(performance) }: PerformanceBenchmarkDependencies,
...
export function performanceBenchmarkJson(result: PerformanceBenchmarkResult): string {
