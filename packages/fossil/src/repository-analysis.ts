...
import type { AnalysisWarning, FossilReport, NormalizedAnalysisOptions, SourceLanguage } from "./types.js";
import {
...
export async function analyzeRepositoryCore(
  ...
  const warnings: AnalysisWarning[] = [
    ...
    ...(submodules.stdout.trim() === ""
      ? []
      : [{ code: "submodule_omitted" as const, message: "Submodule contents are omitted from repository analysis." }]),
  ];
  const bursts = history.assembleClosedBursts(
    includedHistory,
    history.retainQualifiedClosedClusters(
      history.retainClosedTemporalClusters(
        history.splitTemporalClusters(includedHistory, options.gapHours * 60 * 60 * 1_000),
        analysisTimestampMs,
        options.gapHours * 60 * 60 * 1_000,
      ),
    ),
  );
    ...
    const findings = candidates.flatMap((candidate) => {
      ...
      const inbound = new Set(
        graph.edges
          .filter(
            (edge) =>
              edge.targetPath === candidate.path && edge.strength === "strong" && !candidatePaths.has(edge.sourcePath),
          )
          .map((edge) => edge.sourcePath),
      );
      const neighbors = new Set(
        graph.edges.flatMap((edge) =>
          edge.sourcePath === candidate.path
            ? [edge.targetPath]
  ...
  return finalizeFossilReport({
    ...
    completeness: {
      ...
      referenceAnalysisComplete:
        references.graph.complete && !warnings.some((warning) => warning.code === "sparse_checkout"),
