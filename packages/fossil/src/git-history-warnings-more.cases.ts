...
test(
  "reports shallow history without treating malformed Git output as complete",
  () => {
  ...
  assert.deepEqual(gitAnalyzer.shallowHistoryWarnings("true\n"), [
    {
      ...
      message:
        "Repository is shallow; burst and consolidation history may be " +
        "incomplete.",
    },
  ]);
  assert.deepEqual(gitAnalyzer.shallowHistoryWarnings("false\r\n"), []);
  assert.throws(
    () => gitAnalyzer.shallowHistoryWarnings("unknown\n"),
    /Unexpected Git shallow-repository response/,
  );
});

test(
  "reports sparse checkout without treating malformed Git output as complete",
  () => {
  ...
  assert.deepEqual(gitAnalyzer.sparseCheckoutWarnings("true\n"), [
    {
      ...
      message:
        "Sparse checkout is enabled; current-file existence and references " +
        "may be incomplete.",
    },
  ]);
  ...
  assert.throws(
    () => gitAnalyzer.sparseCheckoutWarnings("enabled\n"),
    /Unexpected Git sparse-checkout response/,
  );
});

test(
  "returns empty history evidence and no burst clusters for an empty " +
    "repository",
  () => {
    ...
    assert.deepEqual(
      gitAnalyzer.retainClosedTemporalClusters(temporalClusters, 10_000, 1_000),
      [],
    );
  },
);
