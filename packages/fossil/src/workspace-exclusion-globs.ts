...
function withinCallerLimits(normalized: string, acceptedCount: number, byteLength: number): boolean {
...
function acceptedCallerPattern(pattern: string, acceptedCount: number, byteLength: number): string | undefined {
  const normalized = normalizeWorkspacePath(pattern);
  if (!withinCallerLimits(normalized, acceptedCount, byteLength)) return undefined;
