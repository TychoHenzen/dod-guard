...
function createFixture(): {
  ...
  return {
    directory,
    referenceBytes:
      Buffer.byteLength(tracked) + Buffer.byteLength(oldWorkspace),
  };
}
test(
  "composes tracked and eligible referenced workspace sources into inventory " +
    "evidence",
  async () => {
    const fixture = createFixture();
    try {
      const report = await analyzeRepositoryCore(
        fixture.directory,
        analysisOptions,
      );
