...
import {
  ...
  renderFossilReportTable,
  workspaceDebrisTableRows,
} from "./output.js";
import { createAdvisoryFossilFinding } from "./fossil-grader.js";
import type { BurstReport, FossilFinding, WorkspaceDebrisFinding } from "./types.js";
...
function findingExplanation(candidatePath: string, livePath: string) {
  return {
...
test("escapes controls in whole-report warnings and workspace debris rows", () => {
  ...
  assert.equal(redirected.split("\n").some(hasTerminalControl), false);
  assert.equal(redirected.includes("workspace_unreadable\\u001b\\u0085"), true);
...
test("serializes one complete schema-versioned JSON report without table prose", () => {
  const report = createReport(
    optionsFor("json"),
    { analysisTimestampMs: 1_735_689_600_000, limits: limitsWith(10) },
  );
